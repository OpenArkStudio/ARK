...
TEST_CASE("lz4_compress", "[lz4_compress]")
{
    // compress
    ...
    REQUIRE(true == AFLZ4Compress::Compress(origin_string.c_str(), origin_string.length(), 1, compressed_string));
    auto end = std::chrono::steady_clock::now();
    auto c_interval = std::chrono::duration_cast<std::chrono::microseconds>(end - begin).count();

    begin = std::chrono::steady_clock::now();
    REQUIRE(
        true == AFLZ4Compress::Decompress(compressed_string.c_str(), compressed_string.length(), decompressed_string));
