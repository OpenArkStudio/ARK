...
TEST_CASE("zlib_compress", "[zlib_compress]")
{
    // compress
    ...
    REQUIRE(true == AFZlibCompress::Compress(origin_string.c_str(), origin_string.length(), 1, compressed_string));
    auto end = std::chrono::steady_clock::now();
    auto c_interval = std::chrono::duration_cast<std::chrono::microseconds>(end - begin).count();

    begin = std::chrono::steady_clock::now();
    REQUIRE(
        true == AFZlibCompress::Decompress(compressed_string.c_str(), compressed_string.length(), decompressed_string));
    ...
    std::cout << "zlib testing..." << std::endl;
    std::cout << "origin:\n"
