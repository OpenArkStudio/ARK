...
def write_plugin(res_path,plugin):
    ...
    dir_path = os.path.join(res_path, plugin, plugin_and_module_param.include_path)
    create_folder(dir_path)
    file_name = "AF" + plugin.capitalize() + "Plugin"
    with open(os.path.join(dir_path,file_name + plugin_and_module_param.hpp_ext), 'w') as f:
        f.write(u'''/*
...
ARK_DECLARE_PLUGIN(AF''' + plugin.capitalize() + '''Plugin)
    ...
    # interface
    dir_path = os.path.join(res_path, plugin, plugin_and_module_param.interface_path)
    create_folder(dir_path)
    ...
    create_folder(dir_path)
    file_name = "AF" + plugin.capitalize() + "Plugin"
    with open(os.path.join(dir_path,file_name + plugin_and_module_param.cpp_ext), 'w') as f:
        f.write(u'''/*
 * This source file is part of ARK
 * For the latest info, see https://github.com/ArkNX
 *
...
#include "''' + plugin + '''/include/AF''' + plugin.capitalize() + '''Plugin.hpp"
...
''')
    ...
    # src/dllmain.cpp
    with open(os.path.join(dir_path, plugin_and_module_param.dllmain_file), 'w') as f:
        f.write(u'''/*
 * This source file is part of ARK
 * For the latest info, see https://github.com/ArkNX
 *
...
''')
    ...
    # name/CMakeLists.txt
    with open(os.path.join(res_path, plugin, plugin_and_module_param.cmake_file), 'w') as f:
        f.write(u'''if(WIN32)
...
#include "base/AFPluginManager.hpp"
#include "''' + plugin + '''/interface/AFI''' + module.capitalize()+ '''Module.hpp"

namespace ark {

class AFC''' + module.capitalize() + '''Module : public AFI''' + module.capitalize() + '''Module
{
...
#include "''' + plugin + '''/include/AFC''' + module.capitalize() + '''Module.hpp"
    ...
    for line in lines: #依次读取每行  
        ...
        if flag and line[0:1] != "#":
            if line == "\n":
                ...
                break  
    lines.insert(i-2,"#include \"" + plugin + "/include/AFC" + module.capitalize() + "Module.hpp\"\n")
    ...
    for i in range(i,len(lines)):
        ...
        if lines[i].find("Install") != -1:
            while True:
                ...
                if lines[i].find("}") != -1:
                    break
            lines.insert(i,"\tARK_REGISTER_MODULE(AFI" + module.capitalize() + "Module, AFC" + module.capitalize() + "Module);\n")
            while True:
                if lines[i].find("{") != -1 and lines[i-1].find("Uninstall") != -1:
                    break
                i = i + 1
            i = i + 1
            if lines[i].find("// uninstall module") != -1 :
                i = i + 1
            lines.insert(i, "\tARK_DEREGISTER_MODULE(AFI" + module.capitalize() + "Module, AFC" + module.capitalize() + "Module);\n")
            break
                ...
                if module.isalpha() :
                    ...
                    if plugin_type == "0":
                        ...
                        if int(plugin) in range(1,len(plugin_list)+1) :
                            write_module(module, plugin_list[int(plugin)-1],path)
                            break
                        else :
                            if int(plugin) == 0 :
                                break
