...
public:
    ...
    virtual bool AddClassCallBack(CLASS_EVENT_FUNCTOR &&cb) = 0;
    ...
    bool AddNodeCallBack(const std::string &class_name, const std::string &name, BaseType *pBase,
        int (BaseType::*handler)(const AFGUID &, const std::string &, const AFIData &, const AFIData &))
    {
        ...
        return AddNodeCallBack(class_name, std::move(functor));
    }

    virtual bool AddNodeCallBack(const std::string &name, DATA_NODE_EVENT_FUNCTOR &&cb) = 0;
    virtual bool AddCommonNodeCallback(DATA_NODE_EVENT_FUNCTOR &&cb) = 0;

    // template<typename BaseType>
    // bool AddTableCommonCallback(BaseType* pBase, int (BaseType::*handler)(const AFGUID&, const DATA_TABLE_EVENT_DATA&, const AFIData&,
    // const AFIData&))
    //{
    //    DATA_TABLE_EVENT_FUNCTOR functor = std::bind(handler, pBase, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3,
    //    std::placeholders::_4); return AddTableCommonCallback(std::make_shared<DATA_TABLE_EVENT_FUNCTOR>(functor));
    //}

    virtual bool AddTableCallBack(const std::string &name, DATA_TABLE_EVENT_FUNCTOR &&cb) = 0;
    virtual bool AddCommonTableCallback(DATA_TABLE_EVENT_FUNCTOR &&cb) = 0;
...
public:
    ...
    {
        return AddClassCallBack(class_name, std::make_shared<CLASS_EVENT_FUNCTOR>(std::bind(handler, pBase, std::placeholders::_1,
    ...
    template<typename BaseType>
    bool AddCommonNodeCallback(const std::string &class_name, BaseType *pBase,
        int (BaseType::*handler)(const AFGUID &, const std::string &, const AFIData &, const AFIData &))
    {
        return AddCommonNodeCallback(class_name, std::move(std::bind(handler, pBase, std::placeholders::_1, std::placeholders::_2,
                                                     std::placeholders::_3, std::placeholders::_4)));
    }

    template<typename BaseType>
    bool AddCommonTableCallback(const std::string &class_name, BaseType *pBase,
        int (BaseType::*handler)(const AFGUID &, const DATA_TABLE_EVENT_DATA &, const AFIData &, const AFIData &))
    {
        return AddCommonTableCallback(class_name, std::move(std::bind(handler, pBase, std::placeholders::_1, std::placeholders::_2,
                                                      std::placeholders::_3, std::placeholders::_4)));
    }

    virtual bool DoEvent(
        const AFGUID &entity_id, const std::string &class_name, const ArkEntityEvent class_event, const AFIDataList &args) = 0;
    virtual bool AddNodeCallBack(const std::string &class_name, const std::string &name, DATA_NODE_EVENT_FUNCTOR &&cb) = 0;
    virtual bool AddTableCallBack(const std::string &class_name, const std::string &name, DATA_TABLE_EVENT_FUNCTOR &&cb) = 0;
    virtual bool AddCommonNodeCallback(const std::string &class_name, DATA_NODE_EVENT_FUNCTOR &&cb) = 0;
    virtual bool AddCommonTableCallback(const std::string &class_name, DATA_TABLE_EVENT_FUNCTOR &&cb) = 0;
