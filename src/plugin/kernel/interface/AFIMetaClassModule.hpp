...
#include "AFIDataNodeManager.hpp"
#include "AFIDataTableManager.hpp"
...
public:
    ...
    // template<typename BaseType>
    // bool AddTableCommonCallback(BaseType* pBase, int (BaseType::*handler)(const AFGUID&, const
    // DATA_TABLE_EVENT_DATA&, const AFIData&, const AFIData&))
    //{
    //    DATA_TABLE_EVENT_FUNCTOR functor = std::bind(handler, pBase, std::placeholders::_1, std::placeholders::_2,
    //    std::placeholders::_3, std::placeholders::_4); return
    //    AddTableCommonCallback(std::make_shared<DATA_TABLE_EVENT_FUNCTOR>(functor));
    //}
...
public:
    ...
    bool AddCommonNodeCallback(const std::string& class_name, BaseType* pBase,
        int (BaseType::*handler)(const AFGUID&, const std::string&, const AFIData&, const AFIData&))
    {
        return AddCommonNodeCallback(
            class_name, std::move(std::bind(handler, pBase, std::placeholders::_1, std::placeholders::_2,
    ...
    bool AddCommonTableCallback(const std::string& class_name, BaseType* pBase,
        int (BaseType::*handler)(const AFGUID&, const DATA_TABLE_EVENT_DATA&, const AFIData&, const AFIData&))
    {
        return AddCommonTableCallback(
            class_name, std::move(std::bind(handler, pBase, std::placeholders::_1, std::placeholders::_2,
                            std::placeholders::_3, std::placeholders::_4)));
    }

    virtual bool DoEvent(const AFGUID& entity_id, const std::string& class_name, const ArkEntityEvent class_event,
    ...
    virtual bool InitDataTableManager(
        const std::string& class_name, ARK_SHARE_PTR<AFIDataTableManager> pTableManager) = 0;
