...
public:
    ...
    virtual const std::string GetAppWholeName(bus_id_t bus_id) = 0;
    ...
    virtual bus_id_t GetSelfBusID() = 0;
    ...
    //virtual uint16_t CalcProcPort(bus_id_t bus_id) = 0;

    virtual bool GetTargetBusRelations(std::vector<ARK_APP_TYPE>& target_list) = 0;

    virtual bool GetOtherProc(bus_id_t bus_id, AFProcConfig& proc) const = 0;

    virtual int GetOtherProc(const ARK_APP_TYPE app_type, std::vector<const AFProcConfig*>& proc_list) const = 0;
};
