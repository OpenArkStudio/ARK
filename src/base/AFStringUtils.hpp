...
namespace
{
    ...
    inline void _stringUtilSplitSliceToSlice(
        const ark::AFSlice& str,
        const char& delim,
        std::vector<ark::AFSlice>& ret,
        unsigned int maxSplits)
    {
        // Use STL methods
        size_t start, pos;
    ...
    inline void stringUtil_Split(const StringType& src, StringType& left, StringType& right, typename StringType::const_pointer pDelims, size_t nDelimsLength)
    {
...
namespace ark
{
    ...
    public:
        ...
        static void Split(std::vector<std::string>& ret, const std::string& str, const std::string::value_type& delims, unsigned int max_splits = 0)
        {
            _stringUtilSplit(ret, str, delims, max_splits);
        }

        static void Split(std::vector<std::wstring>& ret, const std::wstring& str, const std::wstring::value_type& delims, unsigned int max_splits = 0)
        {
            _stringUtilSplit(ret, str, delims, max_splits);
        }
