...
namespace ark
{
    ...
    public:
        ...
        AFSlice() : data_(""), size_(0) {}

        // Create a slice that refers to d[0,n-1].
        AFSlice(const char* d, size_t n) : data_(d), size_(n) {}

        // Create a slice that refers to the contents of "s"
        AFSlice(const std::string& s) : data_(s.data()), size_(s.size()) {}

        // Create a slice that refers to s[0,strlen(s)-1]
        AFSlice(const char* s) : data_(s), size_(strlen(s)) {}
