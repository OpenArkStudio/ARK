...
public:
    ...
    uint64_t AddTimer(
        const guid_t& entity_id, uint32_t count, const std ::chrono::milliseconds interval, TIMER_FUNCTOR&& callback)
    {
        ...
        timers_.insert(std::make_pair(timer->idx, timer));
