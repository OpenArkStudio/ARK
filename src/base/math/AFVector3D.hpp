...
public:
    ...
    {
        return IS_FLOAT_ZERO(float, Distance(*this, rhs));
    }
    ...
    bool IsZero() const
    {
        return (IS_FLOAT_ZERO(float, x) && IS_FLOAT_ZERO(float, y) && IS_FLOAT_ZERO(float, z));
    }
    ...
    //    {
    //        ARK_LOG_ERROR("Vector3 FromString failed, error={}, msg={}", ex.code().message(), ex.what());
