...
namespace ark
{
        ...
        for (rapidxml::xml_node<>* pServerNode = pServersNode->first_node(); pServerNode != nullptr; pServerNode = pServerNode->next_sibling())
        {
            ...
            for (rapidxml::xml_node<>* pProcNode = pServerNode->first_node(); pProcNode != nullptr; pProcNode = pProcNode->next_sibling())
            {
                ...
                for (uint8_t i = start; i <= end; ++i)
                {
                    ...
                    server_config.local_ep_ = AFEndpoint::from_string(MakeUrl(protocol, "localhost", port), ec);
#else
                    server_config.local_ep_ = AFEndpoint::from_string(MakeUrl(protocol, "0.0.0.0", port), ec);
#endif
                    const std::string& real_host = GetHost(host_name);
                    server_config.public_ep_ = AFEndpoint::from_string(MakeUrl(protocol, real_host, port), ec);
    ...
    const std::string AFCBusModule::GetAppHost(const int bus_id)
    {
        ...
        return (server_config == nullptr) ? NULL_STR : server_config->local_ep_.to_string();
    }
