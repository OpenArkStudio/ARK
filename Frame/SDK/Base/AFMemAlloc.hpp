...
#define ARK_ALLOC(bytes)                AFMemAlloc::Alloc(bytes, __FILE__, __LINE__)
#define ARK_REALLOC(addr, bytes)        AFMemAlloc::Realloc(addr, bytes, __FILE__, __LINE__)
#define ARK_CALLOC(count, bytes)        AFMemAlloc::Calloc(count, bytes, __FILE__, __LINE__)
#define ARK_FREE(p)                     AFMemAlloc::Free(p)


#else

#define ARK_ALLOC(bytes)                AFMemAlloc::AllocRelease(bytes)
#define ARK_REALLOC(addr, bytes)        AFMemAlloc::ReallocRelease(addr, bytes)
#define ARK_CALLOC(count, bytes)        AFMemAlloc::CallocRelease(count, bytes)
#define ARK_FREE(p)                     AFMemAlloc::FreeRelease(p)
...
public:
    ...
    static void* Alloc(size_t bytes, const char* file, int line)
    {
    ...
    static void* Realloc(void* addr, size_t bytes, const char* file, int line)
    {
        if (g_needRecord)
        {
            if (addr != nullptr)
    ...
    static void* Calloc(size_t count, size_t bytes, const char* file, int line)
    {
        ARK_ASSERT_NO_EFFECT(bytes > 0);
    ...
    //Release
    static void* AllocRelease(size_t bytes)
    {
        ARK_ASSERT_NO_EFFECT(bytes > 0);
    ...
    static void* ReallocRelease(void* addr, size_t bytes)
    {
        ARK_ASSERT_NO_EFFECT(bytes > 0);
    ...
    static void* CallocRelease(size_t count, size_t bytes)
    {
        ARK_ASSERT_NO_EFFECT(bytes > 0);
    ...
    static void FreeRelease(void* p)
    {
        FreeInternal(p);
    }
