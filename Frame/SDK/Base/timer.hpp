...
{
    using timer_handler_t = TimerHandler;
    using  timer_context_ptr = std::shared_ptr<timer_context<timer_handler_t>>;
    ...
    static const uint32_t MAX_TIMER_NUM = UINT32_MAX - 1; //max = power(2, 32)

    using timer_wheel_t = timer_wheel<std::list<uint64_t>, WHEEL_SIZE>;
public:
    timer()
        : tick_(0)
        , prew_tick_(0)
    ...
    */
    ...
    timerid_t repeat(int32_t duration, int32_t times, timer_handler_t&& callBack)
    {
        ...
        auto pt = std::make_shared<timer_context<timer_handler_t>>(std::forward<timer_handler_t>(callBack), duration, times < 0);
        pt->repeattimes(times);
        return add_new_timer(pt);
    }
...
private:
    ...
    {
        t->endtime(t->duration() + millseconds());
        if (t->id() == 0)
        {
