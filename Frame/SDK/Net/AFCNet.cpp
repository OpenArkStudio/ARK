...
std::uint64_t AFCNet::mObjectIndex = 0;

void AFCNet::conn_writecb(struct bufferevent* bev, void* user_data)
{
    //ÿ���յ�������Ϣ��ʱ���¼�
    //  struct evbuffer *output = bufferevent_get_output(bev);
}

void AFCNet::conn_eventcb(struct bufferevent* bev, short events, void* user_data)
{
    NetObject* pObject = (NetObject*)user_data;
    AFCNet* pNet = (AFCNet*)pObject->GetNet();
...
void AFCNet::listener_cb(struct evconnlistener* listener, evutil_socket_t fd, struct sockaddr* sa, int socklen, void* user_data)
{
    //����������
    AFCNet* pNet = (AFCNet*)user_data;
...
void AFCNet::conn_readcb(struct bufferevent* bev, void* user_data)
{
    //���ܵ���Ϣ
    NetObject* pObject = (NetObject*)user_data;
    if (!pObject)
...
void AFCNet::Initialization(const char* strIP, const unsigned short nPort, const int nServerID )
{
    mstrIP = strIP;
    mnPort = nPort;
...
int AFCNet::Initialization(const unsigned int nMaxClient, const unsigned short nPort, const int nServerID, const int nCpuCount)
{
...
bool AFCNet::SendMsgToAllClient(const char* msg, const uint32_t nLen)
{
    if (nLen <= 0)
    {
        return false;
...
bool AFCNet::SendMsg(const char* msg, const uint32_t nLen, const int nSockIndex)
{
    if (nLen <= 0)
    {
        return false;
...
bool AFCNet::SendMsg(const char* msg, const uint32_t nLen, const AFGUID& xClient)
{
    if (nLen <= 0)
    {
        return false;
...
bool AFCNet::SendMsg(const char* msg, const uint32_t nLen, const std::list<int>& fdList)
{
...
bool AFCNet::CloseNetObject(const int nSockIndex)
{
    std::map<int, NetObject*>::iterator it = mmObject.find(nSockIndex);
    if (it != mmObject.end())
    {
...
bool AFCNet::CloseNetObject(const AFGUID& xClient)
{
    for (std::map<int, NetObject*>::iterator it = mmObject.begin(); it != mmObject.end(); ++it)
    {
        NetObject* pObject = it->second;
...
bool AFCNet::Dismantle(NetObject* pObject)
{
...
bool AFCNet::AddNetObject(const int nSockIndex, NetObject* pObject)
{
...
int AFCNet::InitClientNet()
{
    ...
    event_set_log_callback(&AFCNet::log_cb);
    //event_base_loop(base, EVLOOP_ONCE|EVLOOP_NONBLOCK);

    return sockfd;
}

int AFCNet::InitServerNet()
{
...
NetObject* AFCNet::GetNetObject(const int nSockIndex)
{
    std::map<int, NetObject*>::iterator it = mmObject.find(nSockIndex);
    if (it != mmObject.end())
    {
...
void AFCNet::CloseObject(const int nSockIndex)
{
    std::map<int, NetObject*>::iterator it = mmObject.find(nSockIndex);
    if (it != mmObject.end())
    {
...
void AFCNet::log_cb(int severity, const char* msg)
{

}

bool AFCNet::IsServer()
{
    return mbServer;
}

bool AFCNet::Log(int severity, const char* msg)
{
    log_cb(severity, msg);
    return true;
}

bool AFCNet::SendMsgWithOutHead(const int16_t nMsgID, const char* msg, const uint32_t nLen, const int nSockIndex /*= 0*/)
{
...
bool AFCNet::SendMsgWithOutHead(const int16_t nMsgID, const char* msg, const uint32_t nLen, const AFGUID& xClientID)
{
    std::string strOutData;
    int nAllLen = EnCode(nMsgID, msg, nLen, strOutData);
    if (nAllLen == nLen + AFIMsgHead::NF_Head::NF_HEAD_LENGTH)
...
bool AFCNet::SendMsgWithOutHead(const int16_t nMsgID, const char* msg, const uint32_t nLen, const std::list<int>& fdList)
{
    std::string strOutData;
    int nAllLen = EnCode(nMsgID, msg, nLen, strOutData);
    if (nAllLen == nLen + AFIMsgHead::NF_Head::NF_HEAD_LENGTH)
...
bool AFCNet::SendMsgToAllClientWithOutHead(const int16_t nMsgID, const char* msg, const uint32_t nLen)
{
    std::string strOutData;
    int nAllLen = EnCode(nMsgID, msg, nLen, strOutData);
    if (nAllLen == nLen + AFIMsgHead::NF_Head::NF_HEAD_LENGTH)
...
int AFCNet::EnCode(const uint16_t unMsgID, const char* strData, const uint32_t unDataLen, std::string& strOutData)
{
    AFCMsgHead xHead;
...
int AFCNet::DeCode(const char* strData, const uint32_t unAllLen, AFCMsgHead& xHead)
{
    //����--unLenΪbuff�ܳ���,����ʱ���ö����Ƕ���
