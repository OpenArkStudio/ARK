...
std::atomic_uint64_t AFCMulNet::mObjectIndex = 0;
std::atomic_uint64_t AFCMulNet::mThreadExit = 0;

void AFCMulNet::conn_writecb(struct bufferevent* bev, void* user_data)
{
    //ÿ���յ�������Ϣ��ʱ���¼�
    //  struct evbuffer *output = bufferevent_get_output(bev);
}

void AFCMulNet::conn_eventcb(struct bufferevent* bev, short events, void* user_data)
{
    NetObject* pObject = (NetObject*)user_data;
    AFCMulNet* pNet = (AFCMulNet*)pObject->GetNet();
...
void AFCMulNet::listener_cb(struct evconnlistener* listener, evutil_socket_t fd, struct sockaddr* sa, int socklen, void* user_data)
{
    //����������
    AFCMulNet* pNet = (AFCMulNet*)user_data;
...
void AFCMulNet::conn_readcb(struct bufferevent* bev, void* user_data)
{
    //���ܵ���Ϣ
    NetObject* pObject = (NetObject*)user_data;
    if (!pObject)
...
void AFCMulNet::Initialization(const char* strIP, const unsigned short nPort, const int nServerID )
{
    if (!mpNetThread)
    {
        mpNetThread = new std::thread(&AFCMulNet::MulThreadRunClient, this, strIP, nPort, nServerID);
    }
}

int AFCMulNet::Initialization(const unsigned int nMaxClient, const unsigned short nPort, const int nServerID, const int nCpuCount)
{
    if (!mpNetThread)
    {
        mpNetThread = new std::thread(&AFCMulNet::MulThreadRunServer, this, nMaxClient, nPort, nCpuCount, nServerID);
    }

    return 0;
}

void AFCMulNet::MulThreadRunClient(const char* strIP, const unsigned short nPort, const int nServerID /*= 0*/)
{
    mstrIP = strIP;
    mnPort = nPort;
...
void AFCMulNet::MulThreadRunServer(const unsigned int nMaxClient, const unsigned short nPort, const int nCpuCount /*= 4*/, const int nServerID/* = 0*/)
{
...
void AFCMulNet::ProcessMsgLogicThread()
{
    //Handle Msg;
...
bool AFCMulNet::NetThreadCloseNetObject(const int nSockIndex)
{
...
void AFCMulNet::ProcessMsgNetThread()
{
    //Send Msg;
...
void AFCMulNet::ProcessNetTaskNetThread()
{
    NetTaskInfo xMsg;
...
bool AFCMulNet::SendMsgToAllClient(const char* msg, const uint32_t nLen)
{
    if (nLen <= 0)
    {
        return false;
...
bool AFCMulNet::NetThreadSendMsgToAllClient(const char* msg, const uint32_t nLen)
{
    if (nLen <= 0)
    {
        return false;
...
bool AFCMulNet::SendMsg(const char* msg, const uint32_t nLen, const int nSockIndex)
{
    if (nLen <= 0)
    {
        return false;
...
bool AFCMulNet::SendMsg(const char* msg, const uint32_t nLen, const AFGUID& xClient)
{
    if (nLen <= 0)
    {
        return false;
...
bool AFCMulNet::SendMsg(const char* msg, const uint32_t nLen, const std::list<int>& fdList)
{
...
bool AFCMulNet::NetThreadSendMsg(const char* msg, const uint32_t nLen, const int nSockIndex)
{
    if (nLen <= 0)
    {
        return false;
...
bool AFCMulNet::NetThreadSendMsg(const char* msg, const uint32_t nLen, const AFGUID& xClient)
{
    if (nLen <= 0)
    {
        return false;
...
bool AFCMulNet::CloseNetObject(const int nSockIndex)
{
    NetTaskInfo xMsg;
...
bool AFCMulNet::CloseNetObject(const AFGUID& xClientID)
{
    NetTaskInfo xMsg;
    xMsg.nTaskType = NetTaskInfo::NETTASKTYPE_DELETEOBJECT;
...
bool AFCMulNet::Dismantle(NetObject* pObject)
{
    bool bNeedDismantle = false;

    int len = pObject->GetBuffLen();
    if (len > AFIMsgHead::NF_Head::NF_HEAD_LENGTH)
    {
        AFCMsgHead xHead;
        ...
        if (nMsgBodyLength > 0 && xHead.GetMsgID() > 0)
        {
            ...
            xMsg.strMsg = std::string(pObject->GetBuff() + AFIMsgHead::NF_Head::NF_HEAD_LENGTH, nMsgBodyLength);
            xMsg.xClientID = pObject->GetClientID();
...
bool AFCMulNet::NetThreadAddNetObject(const int nSockIndex, NetObject* pObject)
{
...
int AFCMulNet::InitClientNet()
{
    ...
    event_set_log_callback(&AFCMulNet::log_cb);
    //event_base_loop(base, EVLOOP_ONCE|EVLOOP_NONBLOCK);

    return sockfd;
}

int AFCMulNet::InitServerNet()
{
...
NetObject* AFCMulNet::NetThreadGetNetObject(const int nSockIndex)
{
    std::map<int, NetObject*>::iterator it = mmObject.find(nSockIndex);
    if (it != mmObject.end())
    {
...
void AFCMulNet::CloseObject(const int nSockIndex)
{
    std::map<int, NetObject*>::iterator it = mmObject.find(nSockIndex);
    if (it != mmObject.end())
    {
...
void AFCMulNet::log_cb(int severity, const char* msg)
{

}

bool AFCMulNet::IsServer()
{
    return mbServer;
}

bool AFCMulNet::Log(int severity, const char* msg)
{
    log_cb(severity, msg);
    return true;
}

bool AFCMulNet::SendMsgWithOutHead(const int16_t nMsgID, const char* msg, const uint32_t nLen, const int nSockIndex /*= 0*/)
{
...
bool AFCMulNet::SendMsgWithOutHead(const int16_t nMsgID, const char* msg, const uint32_t nLen, const AFGUID& xClientID)
{
    std::string strOutData;
    int nAllLen = EnCode(nMsgID, msg, nLen, strOutData);
    if (nAllLen == nLen + AFIMsgHead::NF_Head::NF_HEAD_LENGTH)
...
bool AFCMulNet::SendMsgWithOutHead(const int16_t nMsgID, const char* msg, const uint32_t nLen, const std::list<int>& fdList)
{
    std::string strOutData;
    int nAllLen = EnCode(nMsgID, msg, nLen, strOutData);
    if (nAllLen == nLen + AFIMsgHead::NF_Head::NF_HEAD_LENGTH)
...
bool AFCMulNet::SendMsgToAllClientWithOutHead(const int16_t nMsgID, const char* msg, const uint32_t nLen)
{
    std::string strOutData;
    int nAllLen = EnCode(nMsgID, msg, nLen, strOutData);
    if (nAllLen == nLen + AFIMsgHead::NF_Head::NF_HEAD_LENGTH)
...
int AFCMulNet::EnCode(const uint16_t unMsgID, const char* strData, const uint32_t unDataLen, std::string& strOutData)
{
    AFCMsgHead xHead;
...
int AFCMulNet::DeCode(const char* strData, const uint32_t unAllLen, AFCMsgHead& xHead)
{
    //����--unLenΪbuff�ܳ���,����ʱ���ö����Ƕ���
