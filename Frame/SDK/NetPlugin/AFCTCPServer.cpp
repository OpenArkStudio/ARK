            ...
            case RECV_DATA:
                {
                    if (net_recv_cb_)
                    {
                        net_recv_cb_(pMsg->head_, pMsg->head_.msg_id(), pMsg->msg_data_.c_str(), pMsg->msg_data_.size(), pEntity->GetConnID());
                    }
                }
                break;
            case CONNECTED:
                net_event_cb_((AFNetEvent)pMsg->event_, pMsg->conn_id_, pMsg->session_->getIP(), bus_id_);
                break;
            case DISCONNECTED:
                {
                    net_event_cb_((AFNetEvent)pMsg->event_, pMsg->conn_id_, pMsg->session_->getIP(), bus_id_);
